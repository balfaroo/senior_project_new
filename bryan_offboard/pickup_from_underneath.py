...
class OffboardControl(Node):
    ...
    def timer_callback(self) -> None:
        ...
        if self.offboard_setpoint_counter == 10: 
            ...
            self.publish_position_setpoint(self.x_local, self.y_local, self.takeoff_height, self.target_heading)
            if abs(self.vehicle_local_position.z - self.takeoff_height) < 0.02 and self.offboard_setpoint_counter < 21:
                self.offboard_setpoint_counter += 1
                self.servo.min()
            if abs(self.vehicle_local_position.z - self.takeoff_height) < 0.02 and self.offboard_setpoint_counter < 30:
                self.offboard_setpoint_counter += 1
